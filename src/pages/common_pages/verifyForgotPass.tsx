...
import { verifyForgotLinkWithRole } from "@/constants/axiosInstance";
import { handleErrors } from "@/util/handleErrors";

const VerfiyForgotPass = React.memo(() => {
  const { token, role }: { token: string; role: "admin"|"user"|"company" } =
    useParams() as { token: string; role: "admin" | "user" | "company" };
    ...
    async function verifyLink() {
      try {
        ...
        const { data } = await axios.post(
          verifyForgotLinkWithRole[role.split("=")[1] as "admin"|"user"|"company"],
          { token },
          { withCredentials: true }
        );
        if (data.status) {
          navigate(`/user/setpassword?role=${role.split("=")[1]}`);
        } else {
...
export default VerfiyForgotPass;
