import { ForogotPasswordVerficationStage } from "@/components/auth/forgoPassCounter";
...
export function ForgotPassword() {
  useEffect(() => {
    const data: { isVerificationState: boolean } | null = JSON.parse(
      localStorage.getItem("forgoVerificationState") ?? "{}"
    );
    setIsLink(data?.isVerificationState === true);
  }, []);
  ...
  return (
    <main className="w-full h-screen flex items-center justify-center relative">
      {isLink ? (
        <ForogotPasswordVerficationStage setIsLink={setIsLink} />
      ) : (
        <div className="w-[90%] sm:w-[60%] md:w-[50%] lg:w-[28%] min-h-96  flex flex-col gap-y-10">
          <div className="w-full flex items-center justify-center flex-col gap-2">
            ...
            <h3 className="company_text text-textPrimary">
              Enter you email address and will send you the link to reset
              password
            </h3>
          </div>
          <div>
