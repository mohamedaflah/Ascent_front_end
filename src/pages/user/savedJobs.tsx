...
import { JobCompanyCard2 } from "@/components/Findjob/JobCard2";
import { Bookmark, CaptionsOff, MapPin, Sparkles } from "lucide-react";

import { useEffect } from "react";
import { getBookMarkJobs, getSpecificJob } from "@/redux/actions/jobActions";

import { Button } from "@/shadcn/ui/button";
import toast from "react-hot-toast";
import NodataImage from "../../assets/undraw_not_found_re_bh2e.svg";
import { ApplyJob } from "@/components/Findjob/ApplyJobModal";
import TechnologyIcon from "@/components/custom/TechIcon";
export function SavedJobs() {
  const { jobs, job,bookmarks } = useSelector((state: RootState) => state.job);
  ...
  useEffect(() => {
    if (user) {
      dispatch(getBookMarkJobs(user?.savedJobs));
    }
  }, [dispatch, user]);
  ...
  const updatedJobs = bookmarks?.map((job) => ({
    ...job,
  ...
  return (
    <main className="w-full min-h-screen overflow-auto flex ">
        {bookmarks&&bookmarks?.length>=1?<>
      <div className="w-full h-full pt-3 pl-3  flex flex-col">
        <div className="w-full flex justify-between">
          <div className="flex flex-col">
            <h1 className="maintxt text-4xl font-bold">Saved jobs</h1>
            <span className="text-textPrimary">
              Showing {bookmarks?.length} result
            </span>
          </div>
        </div>
